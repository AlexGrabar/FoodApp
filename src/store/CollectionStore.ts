...
type Fetcher<Item extends Identifiable, ExternalParams extends object> = (
    params: FetcherParams<ExternalParams>
...
export class CollectionStore<Item extends Identifiable, ExternalParams extends object> {
  ...
  private readonly _fetcher: Fetcher<Item, ExternalParams>;
  private readonly _pageSize: number;

  constructor(fetcher: Fetcher<Item, ExternalParams>, pageSize: number) {
  ...
  private async _fetch(requestOffset: number): Promise<void> {
       ...
       if (!currentParams) {
           runInAction(() => { this._isLoading = false; this._isInitialLoading = false; });
       ...
       try {
           const response = await this._fetcher(fetchParams);
           runInAction(() => {
               const newItems = response.results;
               const uniqueNewItems = newItems.filter(newItem => !this._list.some(existing => existing.id === newItem.id));
